...
class Causal(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim):
        ...
        self.forward_net = nn.Sequential(
            nn.Linear(obs_dim *2 + action_dim, hidden_dim), nn.ReLU(),
...
class ICMAgent(DDPGAgent):
    def __init__(self, icm_scale, crossmodal_scale, **kwargs):
        ...
        if self.obs_type in ('spectrogram_simple', 'spectrogram_noisy'):
            ...
            self.crossmodal_predictor_opt = torch.optim.RAdam(self.crossmodal_predictor.parameters(), lr=self.lr)
        ...
        if self.reward_free:
            ...
            if self.encoder_opt is not None:
                if self.clip_grad_value > 0:
                    torch.nn.utils.clip_grad_norm_(self.encoder.parameters(), self.clip_grad_value)
