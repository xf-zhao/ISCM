...
def to_torch(xs, device):
    return tuple(to_tensor(x, device) for x in xs)
...
def to_tensor(x, device):
    if isinstance(x, Mapping):
        _obs = {}
        for k, v in x.items():
            _obs[k] = torch.as_tensor(v, device=device)
        return _obs
...
class RandomShiftsAug(nn.Module):
    ...
    def forward(self, x):
        ...
        shift *= 2.0 / (
            h + 2 * self.pad
        )  # because range is [-1, 1] + eps, so span is 2.0
...
def fusion_aug(obs):
    #     pixels_aug = RandomShiftsAug(pad=4)(obs["pixels"])
    #     spectrogram_aug = nn.Identity()(obs["spectrogram"])
    #     obs = {"pixels": pixels_aug, "spectrogram": spectrogram_aug}
    #     return obs
    return simple_crossmodal_aug(obs)


def simple_crossmodal_aug(obs):
    pixels_aug = RandomShiftsAug(pad=4)(obs["pixels"])
    spectrogram_aug = obs["spectrogram"]
    obs = {"pixels": pixels_aug, "spectrogram": spectrogram_aug}
    return obs


def noisy_crossmodal_aug(obs):
    return simple_crossmodal_aug(obs)
